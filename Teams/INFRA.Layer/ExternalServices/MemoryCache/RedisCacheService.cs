...
public class RedisCacheService(IDistributedCache _cache, ITeamRepository _teamRepository, ILogger<RedisCacheService> _log) : IRedisCacheService
{
    ...
    private async Task EnsureKeyDoesNotExistAsync(string cacheKey, CancellationToken cancellationToken)
    {
        ...
        {
            LogHelper.Warning($"Key already exists in Redis: {cacheKey}", _log);
    ...
    private async Task<string> EnsureKeyExistsAsync(string cacheKey, CancellationToken cancellationToken)
    {
        ...
        {
            LogHelper.Error($"Key not found in Redis: {cacheKey}", _log);
            return $"Resource with key {cacheKey} not found in redis.";
        }
        return cachedData;
    }
    public async Task StoreArchivedTeamInRedisAsync(TeamDtoModels.TeamDetailsDto redisTeamDto, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            LogHelper.Info($"✅ Team {redisTeamDto.Name} stored in Redis with key {cacheKey}", _log);
            await _teamRepository.RedisDeleteTeamByIdAsync(redisTeamDto.Id, cancellationToken);
            LogHelper.Info($"✅ Team {redisTeamDto.Name} has been deleted from DB successfully.", _log);
        }
        catch (Exception ex)
        {
            LogHelper.Error($"❌ Failed to store team {redisTeamDto.Name} in Redis: {ex.Message}", _log);
            throw;
        }
    }
    ...
    public async Task<TeamDtoModels.TeamDetailsDto> GetArchivedTeamFromRedisAsync(Guid teamId, CancellationToken cancellationToken)
    {
        var cacheKey = BuildKey(teamId.ToString());
        var cachedData = await EnsureKeyExistsAsync(cacheKey, cancellationToken);
        if (cachedData.StartsWith("Resource with key")) return new TeamDtoModels.TeamDetailsDto(
            Id: Guid.Empty,
            Name: string.Empty,
            TeamManagerId: Guid.Empty,
            TeamCreationDate: string.Empty,
            TeamExpirationDate: string.Empty,
            HasAnyProject: false,
            ProjectNames: null,
            State: "NotFound"
        );
        var teamDto = JsonConvert.DeserializeObject<TeamDtoModels.TeamDetailsDto>(cachedData) ?? throw new InvalidOperationException("Deserialized team is null");
        return teamDto;
    }
    ...
    public async Task StoreNewTeamMemberInformationsInRedisAsync(Guid memberId, string teamName, CancellationToken cancellationToken)
    {
        ...
        LogHelper.Info($"✅ Successfully added new entry for key: {cacheKey}", _log);
    }
    public async Task<string> GetNewTeamMemberFromCacheAsync(Guid memberId, CancellationToken cancellationToken)
    {
        ...
        {
            LogHelper.Error($"Dictionary is null or key 'Team Name' missing in cache data for {cacheKey}", _log);
