...
using Teams.INFRA.Persistence.DAL;
...
public static class DependancyInjection
{
    public static IServiceCollection AddInfrastructureDI(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        if (!string.IsNullOrEmpty(conStrings))
            services.AddDbContext<ApplicationDbContext>(opt => opt.UseInMemoryDatabase(conStrings));
    ...
    private static IServiceCollection ManageRedisCacheMemory(this IServiceCollection services,IConfiguration configuration
    )
    {
        ...
        var clientCertificate = new X509Certificate2(certPath, Password, X509KeyStorageFlags.MachineKeySet);
        var options = new ConfigurationOptions
        {
            EndPoints = { Config["Redis:ConnectionString"]! },
            Ssl = true,
            SslHost = "172.29.0.2", //  variabilisé ceci
            Password = Password,
        ...
        options.CertificateValidation += (__, _, chain, sslPolicyErrors) =>
        {
            return sslPolicyErrors == SslPolicyErrors.None
                || (
                    ...
                    && chain!.ChainElements[^1].Certificate.Subject.Equals("CN=Infra local CA")
                );
        };
        ...
        services.AddSingleton<IConnectionMultiplexer>(provider =>
        {
            ...
            {
               throw new RedisConnectionException(ConnectionFailureType.UnableToConnect, ex.Message, ex);
            }
        });
        return services;
    }
