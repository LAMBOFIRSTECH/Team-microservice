...
public class Detail
{
    ...
    public VoState State { get; private set; }
        ...
        VoState state)
    {
        if (string.IsNullOrWhiteSpace(projectName))
            throw new BusinessRuleException("Project name cannot be empty.");

        if (projectStartDate == DateTimeOffset.MinValue)
            throw new BusinessRuleException("Project start date is invalid.");

        if (projectEndDate == DateTimeOffset.MinValue)
            throw new BusinessRuleException("Project end date is invalid.");

        if (projectEndDate <= projectStartDate)
            throw new BusinessRuleException("Project end date must be after start date.");

        if (!Enum.IsDefined(typeof(VoState), state))
            throw new BusinessRuleException("Project state is invalid.");
    ...
    public Detail WithState(VoState newState)
    {
        return new Detail(
            ProjectName,
            ProjectStartDate,
...
public class ProjectAssociation
{
        ...
        List<Detail> details)
    {
        if (projectId == Guid.Empty)
            throw new BusinessRuleException("ProjectId cannot be empty.");

        if (teamManagerId == Guid.Empty)
            throw new BusinessRuleException("TeamManagerId cannot be empty.");

        if (string.IsNullOrWhiteSpace(teamName))
            throw new BusinessRuleException("TeamName cannot be empty.");

        if (details is null || details.Count == 0)
            throw new BusinessRuleException("At least one detail is required.");

        if (details.Count > 3)
            throw new BusinessRuleException("A team cannot have more than 3 projects.");
    ...
    {
        if (detail is null)
            throw new BusinessRuleException("Detail cannot be null.");
    ...
    public void SuspendProject(string projectName)
    {
        var target = _details.FirstOrDefault(d =>
            d.State == VoState.Active &&
        ...
        _details.Add(target.WithState(VoState.Suspended));
    }

    public void RemoveExpiredDetails()
    {
        var expired = _details
            .Where(d => d.ProjectEndDate <= DateTimeOffset.UtcNow)
            .ToList();
    ...
    public IReadOnlyList<Detail> GetExpiredProjects()
    {
        return _details
            .Where(d => d.ProjectEndDate <= DateTimeOffset.UtcNow)
            .ToList();
    ...
    public ProjectAssignmentState ComputedProjectState
    {
        get
        {
            if (_details.Any(d => d.State == VoState.Suspended))
                return ProjectAssignmentState.Suspended;

            if (IsUnderReview)
                return ProjectAssignmentState.UnderReview;

            if (_details.Any(d => d.ProjectEndDate <= DateTimeOffset.UtcNow))
    ...
    public void ValidateProjectAssignmentToTeam(
        TeamName name,
        MemberId teamManagerId,
        DateTimeOffset createdAt)
    {
        if (_details.Count == 0)
            throw new BusinessRuleException("No project details found.");

        if (!_details.Any(d => d.State == VoState.Active))
            throw new BusinessRuleException("At least one active project is required.");

        if (TeamName != name.Value)
            throw new BusinessRuleException("Team name mismatch.");

        if (new MemberId(TeamManagerId) != teamManagerId)
            throw new BusinessRuleException("Team manager mismatch.");

        var first = _details.First();

        if (first.ProjectStartDate < createdAt)
            throw new BusinessRuleException("Project cannot start before team creation.");

        if (first.ProjectStartDate > createdAt.AddDays(7))
            throw new BusinessRuleException("Project must start within 7 days of team creation.");
    }
}
